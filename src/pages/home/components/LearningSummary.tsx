  ...
  return (
    <section className="pb-12">
      <div className="mb-8 flex items-center justify-between">
        ...
        <Link to="/learn" className="text-sm font-medium text-brand-700 hover:text-brand-600" onClick={handleLearnNavigation}>
          进入学习
        </Link>
      </div>
          ...
          {myCourses.map(({ course, enrollment }) => (
            <div key={enrollment.id} className="flex flex-col rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="inline-flex items-center rounded-full border border-brand-100 bg-brand-50 px-2.5 py-1 text-xs font-semibold text-brand-700">
                    学习中
                  </div>
