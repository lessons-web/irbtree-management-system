    ...
    <article className="flex flex-col gap-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm shadow-slate-900/5 transition hover:border-slate-300 hover:shadow-lg lg:flex-row">
      <div className="min-w-0 flex-1">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="space-y-3">
            <div className="flex flex-wrap items-center gap-2">
              <Link to={`/course/${course.code}`} className="text-xl font-bold text-slate-900 transition hover:text-brand-600">
                {course.code}
              </Link>
        ...
        <div className="mt-5 flex flex-wrap items-center gap-3 text-sm text-slate-500">
          <button
            type="button"
            className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-2 transition hover:border-brand-200 hover:text-brand-600 ${course.isLiked ? 'border-brand-100 bg-brand-50 text-brand-600' : 'border-slate-200 bg-white'}`}
            onClick={onLike}
